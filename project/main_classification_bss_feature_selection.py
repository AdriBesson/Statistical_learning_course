...
for n_features in list_n_features:
    ...
    best_model_fwd, best_index_fwd, error_fwd = ut.best_subset_selection(model=model, n_features=n_features, features=features_train.T, targets=labels_train)
...
# Best model
print('Best LDA model - best subset selection: {0}'.format(header[best_index]))
print('Best classification accuracy - BSS: {0}'.format(best_score_test))
print('Best number of features - BSS: {0}'.format(len(best_index)))
print('Best set of features - BSS: {0}'.format(header[best_index]))


plt.figure()
plt.plot(list_n_features, test_error_fwd, label='Best subset selection')
