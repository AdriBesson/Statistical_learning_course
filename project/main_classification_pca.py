...
# Center the data
X  = preproc.scale(features)

# Apply the PCA on the sets of features
pca = dec.PCA(random_state=10)
variance = []
list_components = np.arange(1, features.shape[1], 1)
...
# Find the number of components which explains more than 90% of the variance
n_components = np.where(np.array(variance) > 0.99)[0][0]
pca.set_params(n_components=n_components)
#features = pca.fit_transform(X)
kpca = dec.KernelPCA(n_components=n_components, kernel='sigmoid', n_jobs=6, random_state=10)
features = kpca.fit_transform(X)
#nmf = dec.SparsePCA(n_components=n_components)
# features = nmf.fit_transform(X)

# Create a training set and a test set
features_train, features_test, labels_train, labels_test = model_selection.train_test_split(features, labels, train_size=0.9, test_size=0.1)
...
list_C = np.logspace(10^-3, 10^3, 100)
clf = svm.SVC(random_state=10, kernel='sigmoid')
max_score = 0
best_C = list_C[0]
for C in list_C:
    clf.set_params(C=C)
...
mlp = neural_network.MLPClassifier(hidden_layer_sizes=(80, ), alpha=0.1, activation='relu', solver='adam', batch_size=256, learning_rate='adaptive', max_iter=1000, learning_rate_init=0.001, random_state=10)
